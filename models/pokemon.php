...
$sql = "SELECT p.id, p.name, p.image_url, GROUP_CONCAT(t.name) AS types
        FROM pokemon p
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<div class="pokemon-image"><img data-src="' . $row["image_url"] . '"style="filter: drop-shadow(0 0 0.65rem black);" alt="' . $row["name"] . '"></div>';
